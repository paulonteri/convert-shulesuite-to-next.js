...
describe("Library Dashboard", () => {
    beforeEach(() => {
        ...
        cy.route("POST", API_URL + "library/booksissued").as("addIssueBook"); // /api/v2.0/academics/subject/
    });

    //Library Dasboard
    it("Dashboard", () => {
        cy.visit("/staff/library");
        ...
        //
        cy.url().should("eq", Cypress.config().baseUrl + "/staff/library");
    });

    it("Issue Book", () => {
        cy.visit("/staff/library/issuebookform");
        // check if form
        cy.get(".ant-btn");
        cy.get(
        ...
        cy.url().should(
            "eq",
            Cypress.config().baseUrl + "/staff/library/issuebookform"
        );
    });

    it("Add Book", () => {
        cy.visit("/staff/library/bookinfoform");
        // check if form
        ...
        cy.url().should(
            "eq",
            Cypress.config().baseUrl + "/staff/library/bookinfoform"
        );
    });
});
